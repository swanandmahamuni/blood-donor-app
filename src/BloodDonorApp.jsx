            ...
            {days.map(day => {
              ...
              return (
                <button key={day} onClick={() => pick(day)} style={{
                  padding: "7px 0", borderRadius: 7, border: "none", cursor: "pointer", fontSize: 12, fontFamily: SF,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 10000, overflow: "hidden" }}>
      {[...Array(80)].map((_, i) => {
        ...
        const targetX = xDir * (80 + Math.random() * 850);
        const targetY = -(200 + Math.random() * 850);
        const targetRot = (Math.random() - 0.5) * 1800;
        return (
          <div key={i} style={{
            position: "absolute",
            left: side === 0 ? "-5%" : side === 1 ? "105%" : side === 2 ? "20%" : "80%",
            bottom: "-10%",
            fontSize: 22 + Math.random() * 38,
            userSelect: "none",
            opacity: 0,
...
function RewardModal({ reward, onClose }) {
  useEffect(() => {
    if (reward) {
      try {
        const audio = new Audio('/milestone.mp3');
...
function LoadingSplash({ stage }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: `linear-gradient(135deg,${SB_BG},${SB_BG2},${MAR})`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 99999, fontFamily: AF }}>
      <img src={DEFAULT_LOGO} alt="logo" style={{ width: 180, height: 140, objectFit: "contain", marginBottom: 20 }} />
...
export default function BloodDonorApp() {
  ...
  const card = { background: CARD_BG, border: `1px solid ${BDR}`, borderRadius: 16, overflow: "hidden", boxShadow: `0 4px 20px rgba(139,60,20,0.05), 0 1px 2px rgba(139,60,20,0.03)` };
    ...
    <div style={{ minHeight: "100vh", fontFamily: SF, color: TX_P, ...appBgStyle }} onMouseMove={handleBoardMove} onMouseUp={stopInteract}>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: 64, background: bannerBg, display: "flex", alignItems: "center", zIndex: 200, boxShadow: `0 4px 20px rgba(44,10,4,0.35)` }}>
        <div style={{ width: SW, display: "flex", alignItems: "center", justifyContent: "center", transition: "width 0.32s", flexShrink: 0 }}>
          {bannerIcon
            ...
            : <img src={DEFAULT_LOGO} alt="logo" style={{ width: 54, height: 54, objectFit: "contain", borderRadius: "50%" }} />
          }
        </div>
        <div style={{ flex: 1 }}>
          ...
          {sidebarOpen ? (
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {sidebarIcon
                ...
                : <img src={DEFAULT_LOGO} alt="logo" style={{ width: 56, height: 56, objectFit: "contain", borderRadius: 10, boxShadow: `0 4px 14px ${SAF_G}` }} />
              }
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              {sidebarIcon
                ...
                : <img src={DEFAULT_LOGO} alt="logo" style={{ width: 42, height: 42, objectFit: "contain", borderRadius: 8, boxShadow: `0 4px 12px ${SAF_G}` }} />
              }
            </div>
          )}
          {sidebarOpen && (
        ...
        <div style={{ padding: "8px", borderTop: "1px solid rgba(255,255,255,0.06)", position: "relative", zIndex: 1 }}>
          ...
          {sidebarOpen && (
            <div style={{ paddingLeft: 8, marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", fontFamily: SF, paddingLeft: 13, marginTop: 2 }}>Crafted by Swanandsinh Ashok Mahamuni & Priyaveera Hemant Khadsare</div>
            </div>
          )}
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 16 }}>
              {[
                ...
                { label: "ACCEPTED", value: stats.accepted, color: GRN, iconBg: "rgba(5,150,105,0.1)", icon: "✅", onClick: () => handleKPIClick("Accepted") },
                { label: "REJECTED", value: stats.rejected, color: RED, iconBg: "rgba(220,38,38,0.1)", icon: "❌", onClick: () => handleKPIClick("Rejected") },
              ].map(s => (
                <div key={s.label} onClick={s.onClick} className="kpi-card" style={{ ...card, padding: "24px 26px", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 16, marginBottom: 20 }}>
              {[
                ...
                { label: "OLD SHRADDHAVAN", value: stats.oldS, color: PURPLE, iconBg: "rgba(124,58,237,0.1)", icon: "🔄", onClick: () => handleKPIClick("All", "Old") },
              ].map(s => (
                <div key={s.label} onClick={s.onClick} className="kpi-card" style={{ ...card, padding: "24px 26px", position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg,transparent,${s.color}60,transparent)` }} />
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {Object.entries(stats.byGender).map(([g, c]) => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: 10, color: TX_M, minWidth: 32, textAlign: "right" }}>{stats.total ? Math.round(c / stats.total * 100) : 0}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={card}>
              ...
              {donors.length === 0 ? (
                <div style={{ padding: 48, textAlign: "center", color: TX_M }}>
                  <img src={DEFAULT_LOGO} alt="logo" style={{ width: 100, height: 100, objectFit: "contain", marginBottom: 12 }} />
                  ...
                  <div style={{ marginBottom: 18 }}><label style={lS}>Status *</label>
                    <div style={{ display: "flex", gap: 10 }}>
                      {["Accepted", "Rejected"].map(s => (
                        <button key={s} onClick={() => handleFormChange("status", s)} style={{
                          flex: 1, padding: 14, borderRadius: 10, cursor: "pointer", fontSize: 14, fontFamily: SF, fontWeight: "700", transition: "all 0.2s",
      ...
      <style>{`
        ...
        @keyframes celebrateBurst { 
          ...
          12% { opacity: 1; transform: translate(calc(var(--tx) * 0.08), calc(var(--ty) * 0.08)) scale(1.6) rotate(calc(var(--tr) * 0.08)); }
        ...
        @keyframes rewardShine { 0% { transform: translate(-50%, -50%) rotate(0deg); } 100% { transform: translate(-50%, -50%) rotate(360deg); } }
        @keyframes goldenGlowPulse {
          0%, 100% { box-shadow: 0 0 40px rgba(255, 215, 0, 0.3), 0 40px 120px rgba(0,0,0,0.9); border-color: rgba(255, 215, 0, 0.4); }
          50% { box-shadow: 0 0 80px rgba(255, 215, 0, 0.6), 0 40px 120px rgba(0,0,0,0.9); border-color: rgba(255, 215, 0, 0.8); }
        }
        @keyframes backupSlide { from{opacity:0;transform:translateX(-40px)} to{opacity:1;transform:translateX(0)} }
        .kpi-card { transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1); cursor: pointer; border: 1px solid ${BDR}!important; }
        .kpi-card:hover { transform: translateY(-6px); box-shadow: 0 16px 36px rgba(139,60,20,0.12), 0 4px 8px rgba(139,60,20,0.06)!important; border-color: ${SAF}40!important; background: linear-gradient(180deg, #fff, ${APP_BG2}20)!important; }
        .kpi-card:active { transform: translateY(-2px); transition-duration: 0.1s; }
        .donut-slice { transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); cursor: pointer; }
        @keyframes chartEntry { 
          0% { transform: rotate(-270deg) scale(0.2); opacity: 0; }
